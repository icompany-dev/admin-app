...
import { useCompanyOfficialSealStore } from "~/stores/CompanyOfficialSeals"
import { useCompanyStore } from "~/stores/Companies"
import { Company } from "~/scripts/models/Company"
import { CompanyConstants } from "~/scripts/constants/Company"
import { PropsResolutionDocument } from "~/scripts/props/PropsResolutionDocument"

export class UseCommonSealAbroadController
  extends ServiceController
  implements IServiceController<CompanyOfficialSeal, ReturnType<typeof useCompanyOfficialSealStore>>
{
  application: CompanyOfficialSeal = new CompanyOfficialSeal()
  applicationId: string | null = null
  repository = useCompanyOfficialSealStore()
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new CompanyOfficialSeal(response)
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new CompanyOfficialSeal()
  ...
  async onSubmitClicked(): Promise<void> {
    if (this.isADirector.value) {
      if (this.dcrRef) {
        ...
        this.application = new CompanyOfficialSeal(updatedData)
