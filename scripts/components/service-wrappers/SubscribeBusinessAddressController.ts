...
import { useCompanyAddressSubscriptionStore } from "~/stores/CompanyAddressSubscriptions"
import { useCompanyStore } from "~/stores/Companies"
import { Company } from "~/scripts/models/Company"
import { CompanyConstants } from "~/scripts/constants/Company"

export class SubscribeBusinessAddressController
  extends ServiceController
  implements IServiceController<CompanyAddressSubscription, ReturnType<typeof useCompanyAddressSubscriptionStore>>
{
  application: CompanyAddressSubscription = new CompanyAddressSubscription()
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new CompanyAddressSubscription(response)
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new CompanyAddressSubscription()
  ...
  async onSubmitClicked(): Promise<void> {
    if (this.dcrRef) {
      ...
      this.application = new CompanyAddressSubscription(updatedData)
    }
