...
import { useCompanyBankAccountClosureStore } from "~/stores/CompanyBankAccountClosures"
...
export class CloseBankAccountController
  extends ServiceController
  implements IServiceController<CompanyBankAccountClosure, ReturnType<typeof useCompanyBankAccountClosureStore>>
{
  application: CompanyBankAccountClosure = new CompanyBankAccountClosure()
  applicationRef = ref<CompanyBankAccountClosure>(new CompanyBankAccountClosure())
  applicationId: string | null = null
  repository = useCompanyBankAccountClosureStore()
  companyRepository = useCompanyStore()

  showMcrFirst = ref<boolean>(false)

  constructor(companyId: string, emitEvents: any | null, applicationId: string | null = null) {
    super(CompanyConstants.TARGET_CLOSE_BANK_ACCOUNT, companyId, emitEvents)
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new CompanyBankAccountClosure(response)
      this.applicationRef.value = new CompanyBankAccountClosure(response)
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new CompanyBankAccountClosure()
  ...
  async onSubmitClicked(): Promise<void> {
    if (this.dcrRef) {
      ...
      this.application = new CompanyBankAccountClosure(updatedData)
  ...
  get resolutionDocumentProps() {
    let props = new PropsResolutionDocument<CompanyBankAccountClosure>(
      this.companyId,
      this.applicationId,
      this.applicationRef.value as CompanyBankAccountClosure,
      this.isDraft(),
      "Draft",
      false,
      false
    ...
    if (this.applicationRef.value && !StringUtil.isNullOrEmpty(this.applicationRef.value.companyBankId)) {
      props.companyBankId = this.applicationRef.value.companyBankId
    }
