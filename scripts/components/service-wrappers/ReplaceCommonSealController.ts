...
import { useCompanyCommonSealReplacementStore } from "~/stores/CompanyCommonSealReplacements"
import { useCompanyStore } from "~/stores/Companies"
import { Company } from "~/scripts/models/Company"
import { CompanyConstants } from "~/scripts/constants/Company"

export class ReplaceCommonSealController
  extends ServiceController
  implements IServiceController<CompanyCommonSealReplacement, ReturnType<typeof useCompanyCommonSealReplacementStore>>
{
  application: CompanyCommonSealReplacement = new CompanyCommonSealReplacement()
  applicationId: string | null = null
  repository = useCompanyCommonSealReplacementStore()
  companyRepository = useCompanyStore()

  showMcrFirst = ref<boolean>(false)

  constructor(companyId: string, emitEvents: any | null, applicationId: string | null = null) {
    super(CompanyConstants.TARGET_COMMON_SEAL, companyId, emitEvents)
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new CompanyCommonSealReplacement(response)
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new CompanyCommonSealReplacement()
  ...
  async onSubmitClicked(): Promise<void> {
    if (this.isADirector.value) {
      if (this.dcrRef) {
        ...
        this.application = new CompanyCommonSealReplacement(updatedData)
