...
import { useCompanyDirectorManagerAppointmentStore } from "~/stores/CompanyDirectorManagerAppointments"
...
{
  application: CompanyDirectorManagerAppointment = new CompanyDirectorManagerAppointment()
  applicationRef = ref<CompanyDirectorManagerAppointment>(new CompanyDirectorManagerAppointment())
  ...
  constructor(companyId: string, emitEvents: any | null, applicationId: string | null = null) {
    super(CompanyConstants.TARGET_DIRECTOR_MANAGER_APPOINTMENT, companyId, emitEvents)
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new CompanyDirectorManagerAppointment(response)
      this.applicationRef.value = new CompanyDirectorManagerAppointment(response)
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new CompanyDirectorManagerAppointment()
  ...
  async onSubmitClicked(): Promise<void> {
    if (this.dcrRef) {
      ...
      this.application = new CompanyDirectorManagerAppointment(updatedData)
