...
import { useCompanyContractEnterStore } from "~/stores/CompanyContractEnters"
import { useCompanyStore } from "~/stores/Companies"
import { Company } from "~/scripts/models/Company"
import { CompanyConstants } from "~/scripts/constants/Company"
import { PropsResolutionDocument } from "~/scripts/props/PropsResolutionDocument"

export class EnterContractController
  extends ServiceController
  implements IServiceController<CompanyContractEnter, ReturnType<typeof useCompanyContractEnterStore>>
{
  application: CompanyContractEnter = new CompanyContractEnter()
  applicationId: string | null = null
  repository = useCompanyContractEnterStore()
  companyRepository = useCompanyStore()

  showMcrFirst = ref<boolean>(false)

  constructor(companyId: string, emitEvents: any | null, applicationId: string | null = null) {
    super(CompanyConstants.TARGET_CONTRACT_ENTER, companyId, emitEvents)
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new CompanyContractEnter(response)
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new CompanyContractEnter()
  ...
  async onSubmitClicked(): Promise<void> {
    if (this.isADirector.value) {
      if (this.dcrRef) {
        ...
        this.application = new CompanyContractEnter(updatedData)
