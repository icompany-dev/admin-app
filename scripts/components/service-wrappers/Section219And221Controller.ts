...
import { useDirectorDeclarationConflictOfInterestStore } from "~/stores/DirectorDeclarationConflictOfInterests"
import { useCompanyStore } from "~/stores/Companies"
import { Company } from "~/scripts/models/Company"
import { CompanyConstants } from "~/scripts/constants/Company"

export class Section219And221Controller
  extends ServiceController
  implements
    IServiceController<
      DirectorDeclarationConflictOfInterest,
      ReturnType<typeof useDirectorDeclarationConflictOfInterestStore>
    >
{
  application: DirectorDeclarationConflictOfInterest = new DirectorDeclarationConflictOfInterest()
  ...
  constructor(companyId: string, emitEvents: any | null, applicationId: string | null = null) {
    super(CompanyConstants.TARGET_DIRECTOR_DECLARATION_CONFLICT_OF_INTEREST, companyId, emitEvents)
  ...
  async fetchApplication(id: string): Promise<void> {
    ...
    if (!this.repository.error) {
      this.application = new DirectorDeclarationConflictOfInterest(response)
      this.applicationId = this.application.id
      this.targetId = this.application.id
      this.directorId.value = this.application.directorId
    }
  }
  ...
  async setApplication(companyId: string): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application = new DirectorDeclarationConflictOfInterest()
  ...
  getDirectorId(): string {
    if (!StringUtil.isNullOrEmpty(this.application.id)) {
      return this.application.directorId
    }

    return this.directorId.value ?? ""
  }
  ...
  async onSubmitClicked(): Promise<void> {
    try {
      if (this.dcrRef) {
        ...
        this.application = new DirectorDeclarationConflictOfInterest(updatedData)
  ...
  override async submitSignature(): Promise<void> {
    ...
    let dayjs = useDayjs()
    await this.application.submitSignature(
      this.signatureFile.value ?? "",
      this.time.formatDateOnlySystem(dayjs().format("YYYY-MM-DD")),
      useFileStore(),
      this.repository
    )
  }
