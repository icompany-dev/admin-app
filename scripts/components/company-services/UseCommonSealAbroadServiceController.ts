...
export class UseCommonSealAbroadServiceController extends CompanyServiceController<CompanyOfficialSeal> {
  companyOfficialSeal = ref<CompanyOfficialSeal>(new CompanyOfficialSeal())

  wrapperRef: any | null = null

  constructor(companyId: string, viewType: string, emitEvents: any | null) {
    super(companyId, true, false, CompanyOfficialSeal, useCompanyOfficialSealStore(), emitEvents)
    this.target = CompanyConstants.TARGET_OFFICIAL_SEAL
  ...
  async initializeData(): Promise<void> {
    switch (this.viewType.value) {
      case ViewMode.New:
        ...
        this.companyOfficialSeal.value = new CompanyOfficialSeal(this.companyServiceInitializer.newApplication)
        ...
        if (this.companyServiceInitializer.existingApplication) {
          ...
          this.companyOfficialSeal.value = new CompanyOfficialSeal(this.companyServiceInitializer.existingApplication)
        }
        break
    ...
    this.init(this.companyOfficialSeal.value as CompanyOfficialSeal)
  }
  ...
  async fetchOngoingApplication(): Promise<void> {
    try {
      ...
      if (apiRecord.totalRecords <= 0) {
        this.companyOfficialSeal.value = new CompanyOfficialSeal()
        this.companyOfficialSeal.value.companyId = this.companyId
        this.hasOngoingApplication.value = false
        return
      }

      this.companyOfficialSeal.value = new CompanyOfficialSeal(apiRecord.data[0])
      this.isInPreviewMode.value = false
      this.hasOngoingApplication.value = true
    } catch (error) {
      if (error instanceof Error) {
        ...
        let errorMessage = new Error(Error.ERROR_TYPE_API, "unable to fetch ongoing application")
        errorMessage.handle()
      }
    }
  }
  ...
  async fetchPreviousSubmission(): Promise<void> {
    try {
      ...
      let lastApplication = new CompanyOfficialSeal(apiRecord.data[0])
      ...
      if (error instanceof Error) {
        ...
        let errorMessage = new Error(Error.ERROR_TYPE_API, "Unable to fetch application")
        errorMessage.handle()
      }
    }
  }

  async onApplicationUpdated(application: CompanyOfficialSeal): Promise<void> {
    await this.fetchOngoingApplication()

    if (this.dcrRef) {
      this.dcrRef.updateApplicationContent(this.companyOfficialSeal.value)
    }
  }

  setApplicationData(applicationData: CompanyOfficialSeal): void {
    if (!applicationData) {
      return
    }
  ...
  async makePayment(): Promise<void> {
    ...
    try {
      ...
      if (StringUtil.isNullOrEmpty(this.companyOfficialSeal.value.id)) {
      ...
      let makePayment = new MakePayment(
        ...
        this.companyId,
        this.target,
        this.companyOfficialSeal.value.id
      )
  ...
  async submitApplication(): Promise<void> {
    if (StringUtil.isNullOrEmpty(this.companyOfficialSeal.value.id)) {
      this.companyOfficialSeal.value.companyId = this.companyId
      await this.companyOfficialSeal.value.create(useCompanyOfficialSealStore())
    } else {
      await this.companyOfficialSeal.value.update(useCompanyOfficialSealStore())
    }
  }

  async onProceedClicked(): Promise<void> {
    if (StringUtil.isNullOrEmpty(this.companyOfficialSeal.value.id) || !this.hasPaid()) {
      this.makePayment()
      return
    }
  ...
  slipCaseTitle(): string {
    return this.language.isMalay()
      ? "Resolusi: Penggunaan Metera Umum di Luar Negara"
      : "Resolution: Official Seal for Use Abroad"
  }
  ...
  get serviceWrapperProps() {
    ...
    return new PropsCompanyServiceWrapper(
      this.companyOfficialSeal.value,
      this.companyId,
      this.target,
      this.slipCaseTitle(),
      this.viewType.value,
      ...
      CompanyOfficialSeal,
      useCompanyOfficialSealStore()
    )
  }

  get resolutionDocumentProps() {
    return new PropsResolutionDocument<CompanyOfficialSeal>(
      this.companyId,
      this.companyOfficialSeal.value.id,
      null,
      this.showWatermark(),
