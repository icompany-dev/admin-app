...
export class AppointChairmanServiceController extends CompanyServiceController<CompanyDirectorManagerAppointment> {
  ...
  private servicePricingId: string = "a46a69f2-61f3-4766-ae8a-28da550f6137"
  ...
  constructor(companyId: string, viewType: string, emitEvents: any | null) {
    ...
    this.target = CompanyConstants.TARGET_DIRECTOR_CHAIRMAN_APPOINTMENT
  ...
  override async fetchPrice(): Promise<void> {
    try {
      ...
      let response = await repository.fetch(this.servicePricingId)
      if (repository.error) {
        throw repository.error
      }

      this.servicePricing.value = new ServicePricing(response)
      this.price.value = this.servicePricing.value.baseGrandTotal
    } catch (e) {
      this.price.value = 19
      console.error(e)
    }
  ...
  async fetchOngoingApplication(): Promise<void> {
    try {
      ...
      let applicationForChairman = apiRecord.data.find((record: any) => {
        let appointment = new CompanyDirectorManagerAppointment(record)
        return appointment.roleName === "Chairman"
      })

      if (apiRecord.totalRecords <= 0 || !applicationForChairman) {
        this.companyDirectorManagerAppointment.value = new CompanyDirectorManagerAppointment()
        this.companyDirectorManagerAppointment.value.companyId = this.companyId
        this.companyDirectorManagerAppointment.value.roleName = "Chairman"
        this.hasOngoingApplication.value = false
        return
      }

      this.companyDirectorManagerAppointment.value = new CompanyDirectorManagerAppointment(applicationForChairman)
  ...
  async makePayment(): Promise<void> {
    ...
    try {
      ...
      let makePayment = new MakePayment(
        ...
        this.companyId,
        CompanyConstants.TARGET_DIRECTOR_MANAGER_APPOINTMENT,
      ...
      let paymentCartItem = makePayment.paymentCart.items.find((pci: PaymentCartItem) => {
        return (
          pci.targetId === this.companyDirectorManagerAppointment.value.id &&
          pci.targetType === CompanyConstants.TARGET_DIRECTOR_MANAGER_APPOINTMENT
        )
      })

      if (paymentCartItem) {
        paymentCartItem.servicePricingId = this.servicePricingId
  ...
  slipCaseTitle(): string {
    return this.language.isMalay() ? "Resolusi: Lantik Pengerusi" : "Resolution: Appoint Chairperson"
  }
