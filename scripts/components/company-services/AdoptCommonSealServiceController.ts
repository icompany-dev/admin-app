...
export class AdoptCommonSealServiceController extends CompanyServiceController<CompanyCommonSeal> {
  companyCommonSeal = ref<CompanyCommonSeal>(new CompanyCommonSeal())

  wrapperRef: any | null = null

  constructor(companyId: string, viewType: string, emitEvents: any | null) {
    super(companyId, true, false, CompanyCommonSeal, useCompanyCommonSealStore(), emitEvents)
  ...
  async initializeData(): Promise<void> {
    switch (this.viewType.value) {
      case ViewMode.New:
        ...
        this.companyCommonSeal.value = new CompanyCommonSeal(this.companyServiceInitializer.newApplication)
        ...
        if (this.companyServiceInitializer.existingApplication) {
          ...
          this.companyCommonSeal.value = new CompanyCommonSeal(this.companyServiceInitializer.existingApplication)
        }
        break
    ...
    this.init(this.companyCommonSeal.value as CompanyCommonSeal)
  }
  ...
  async fetchOngoingApplication(): Promise<void> {
    try {
      ...
      if (apiRecord.totalRecords <= 0) {
        this.companyCommonSeal.value = new CompanyCommonSeal()
        this.companyCommonSeal.value.companyId = this.companyId
        this.hasOngoingApplication.value = false
        return
      }

      this.companyCommonSeal.value = new CompanyCommonSeal(apiRecord.data[0])
      this.isInPreviewMode.value = false
      this.hasOngoingApplication.value = true
    } catch (error) {
  ...
  async fetchPreviousSubmission(): Promise<void> {
    try {
      ...
      let lastApplication = new CompanyCommonSeal(apiRecord.data[0])
  ...
  async onApplicationUpdated(application: CompanyCommonSeal): Promise<void> {
  ...
  setApplicationData(applicationData: CompanyCommonSeal): void {
    if (!applicationData) {
      return
    }
  ...
  async submitApplication(): Promise<void> {
    if (StringUtil.isNullOrEmpty(this.companyCommonSeal.value.id)) {
      this.companyCommonSeal.value.companyId = this.companyId
      await this.companyCommonSeal.value.create(useCompanyCommonSealStore())
    } else {
      await this.companyCommonSeal.value.update(useCompanyCommonSealStore())
    }
  }
  ...
  slipCaseTitle(): string {
    return this.language.isMalay() ? "Resolusi: Penggunaan Metera Umum" : "Resolution: Adopt Common Seal"
  }
  ...
  get serviceWrapperProps() {
    ...
    return new PropsCompanyServiceWrapper(
      ...
      CompanyCommonSeal,
      useCompanyCommonSealStore()
    )
  }

  get resolutionDocumentProps() {
    return new PropsResolutionDocument<CompanyCommonSeal>(
      this.companyId,
      this.companyCommonSeal.value.id,
      null,
      this.showWatermark(),
