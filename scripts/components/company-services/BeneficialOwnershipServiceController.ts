...
export class BeneficialOwnershipServiceController extends CompanyServiceController<CompanyBODeclaration> {
  ...
  isShowFudter(): boolean {
    return (
      this.hasExistingDeclaration.value &&
      this.viewType.value === ViewMode.Existing &&
      !StringUtil.isNullOrEmpty(this.beneficialOwnership.value.signatureFileId)
    )
  }
  ...
  async submitApplication(): Promise<void> {
    if (StringUtil.isNullOrEmpty(this.beneficialOwnership.value.id)) {
      this.beneficialOwnership.value.companyId = this.companyId
      this.beneficialOwnership.value.shareholderId = this.shareholderId.value ?? ""
      await this.beneficialOwnership.value.create(useCompanyBODeclarationStore())
    } else {
      await this.beneficialOwnership.value.update(useCompanyBODeclarationStore())
    }
  }

  async onProceedClicked(): Promise<void> {
    if (!this.hasExistingDeclaration.value) {
      // First one is free
      await this.submitApplication()
    ...
    if (StringUtil.isNullOrEmpty(this.beneficialOwnership.value.id) || !this.hasPaid()) {
  ...
  setApplicationData(applicationData: CompanyBODeclaration): void {
    if (!applicationData) {
      return
    }

    this.beneficialOwnership.value = new CompanyBODeclaration(applicationData)
  }

  async onApplicationUpdated(application: CompanyBODeclaration): Promise<void> {
    this.beneficialOwnership.value = new CompanyBODeclaration(application)
