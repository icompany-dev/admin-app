...
export class AdoptConstitutionServiceController extends CompanyServiceController<CompanyAmendmentConstitution> {
  ...
  constructor(companyId: string, viewType: string, emitEvents: any | null) {
    super(companyId, false, true, CompanyAmendmentConstitution, useCompanyAmendmentConstitutionStore(), emitEvents)
  ...
  async initializeData(): Promise<void> {
    switch (this.viewType.value) {
      ...
      case ViewMode.Existing:
        ...
        // NOTE: Breaking away because the function is too customized
  ...
  slipCaseTitle(): string {
    let items = []
    if (this.language.isMalay()) {
      items = ["Resolusi: Cadangan Terima Pakai Perlembagaan.", "Resolusi: Terima Pakai Perlembagaan."]
    }
