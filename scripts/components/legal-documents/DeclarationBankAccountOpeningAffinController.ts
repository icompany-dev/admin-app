...
export class DeclarationBankAccountOpeningAffinController extends SdnBhdLegalDocumentController {
  ...
  async setSignatureItems(): Promise<void> {
    ...
    this.signatureItems.value = this.directors.value.map((d: Director) => {
      ...
      return new SignatureItem(
        signature?.signature?.url ?? null,
        signature !== null,
        this.isDocumentEditable() && d.email === user.email,
        false,
        d.name,
        d.email,
        "Director",
