...
import { CompanyDocumentRequestItem } from "~/scripts/models/CompanyDocumentRequestItem"
import { DocumentPurchaser } from "~/scripts/library/DocumentPurchaser"
import { Company } from "~/scripts/models/Company"

export class CreateDocumentRequestController extends BasePopupController {
  companyId: Ref<string> = ref<string>("")

  isRequestForPayment: Ref<boolean> = ref<boolean>(true)
  companyDocumentRequest = ref<CompanyDocumentRequest>(new CompanyDocumentRequest())
  ...
  companies: Ref<Company[]> = ref<Company[]>([])
  ...
  async fetchCompanies(): Promise<void> {
    try {
      ...
      let response = await repository.fetchCompact("take_all=1")
      this.companies.value = response.map((c: any) => {
  ...
  async uploadFiles(): Promise<void> {
    ...
    try {
      ...
      if (this.isRequestForPayment.value) {
        let newRequestItems = this.uploadedFiles.value.map((f: UploadedFile) => {
          let newItem = new CompanyDocumentRequestItem()
          newItem.documentName = f.name
          newItem.iCompanyFileId = f.id
          newItem.uploadedFileId = f.id

          return newItem
        })

        let documentPurchaser = new DocumentPurchaser(this.companyId.value)
        documentPurchaser.setItems(newRequestItems)
        await documentPurchaser.purchase()
      }
  ...
  get heading(): string {
    if (this.language.isMalay()) {
      return `Terbitkan Permohonan Dokumen`
    }

    return `Create Document Requests`
  }
  ...
  get requestPaymentLabel(): string {
    return this.language.isMalay() ? "Keluarkan Perintah Bayaran" : "Issue Payment Order"
  }

  get companyOptions(): SelectOption[] {
    return this.companies.value.map((c: Company) => {
