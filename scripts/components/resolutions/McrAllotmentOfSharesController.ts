...
export class McrAllotmentOfSharesController extends ResolutionController<CompanyShareholderAllotment> {
  ...
  // NOTE: This is to capture data changes from the DCR
  // -- DCR can update the data for allot tos
  ...
  changeAllotToType(allotTo: AllotTo): void {
    ...
    if (allotTo.type === AlloteeType.Existing) {
      ...
      allotTo.allotee.shareholderName = ""
      allotTo.allotee.shareholderIdentification = ""
      allotTo.allotee.shareholderIdentificationType = ""
    } else {
      allotTo.allotee.shareholderId = null
      allotTo.allotee.shareholder = null
    }
  ...
  updateAllotTos(newAllotTos: AllotTo[]): void {
    if (ObjectUtil.isArrayEqual<AllotTo>(newAllotTos, this.allotTos.value)) {
      return
    }

    this.allotTos.value = newAllotTos.map((d: AllotTo) => {
      return new AllotTo(d.type, d.allotee, d.isDisabled)
    })
  }
