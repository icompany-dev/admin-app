...
export class DcrAuditCirculationController extends ResolutionController<CompanyAuditCirculation> {
  companyAuditCirculationRepository = useCompanyAuditCirculationStore()
  ...
  private documentTemplateId: string = "d2e688e4-eb17-4220-8c5a-5a52dbf32792"

  time = useLocalTime()

  constructor(props: IPropsResolutionDocument<CompanyAuditCirculation>, emitEvents: any | null) {
    super(
      props.companyId,
      props.applicationId,
      props.application,
  ...
  async fetchApplication(id: string): Promise<void> {
    let response = await this.companyAuditCirculationRepository.fetch(id)
    if (!this.companyAuditCirculationRepository.error && response !== null) {
      this.application.value = new CompanyAuditCirculation(response)
    }
  }
  ...
  async setApplication(): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application.value = new CompanyAuditCirculation()
  ...
  getContent(): string {
    ...
    this.documentTemplate.value.content = this.documentTemplate.value.content.replaceAll(
      "$date.&lt;name=financialYearEndDate&gt;$",
    ...
    if (this.isInPreviewMode.value) {
      if (this.application.value) {
        if (StringUtil.isNullOrEmpty(this.application.value.financialYearEndDate)) {
          this.application.value.financialYearEndDate = "YOUR FINANCIAL YEAR END DATE"
        }
      }
  ...
  get financialYearEndDate(): string {
    if (!this.application.value || !this.application.value.financialPeriod) {
      return `<span class='value-placeholder'>YOUR FINANCIAL YEAR END DATE</span>`
    }

    let time = useLocalTime()
    return time.formatDateOnlyFull(this.application.value.financialPeriod.endDate).toUpperCase()
  }

  get financialYearStartDate(): string {
    if (!this.application.value || !this.application.value.financialPeriod) {
      return `<span class='value-placeholder'>YOUR FINANCIAL YEAR START DATE</span>`
    }

    let time = useLocalTime()
    return time.formatDateOnlyFull(this.application.value.financialPeriod.startDate)
  }
}
