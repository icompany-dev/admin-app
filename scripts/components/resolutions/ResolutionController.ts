...
export abstract class ResolutionController<T> {
  ...
  isUsingTemplate = ref<boolean>(false)
  ...
  constructor(
    ...
    this.isMcr.value = isMcr
    this.setShowWatermark(showWatermark)
    this.setWatermarkText(watermarkText)
    this.initializeResolution(isInPreviewMode)
  }

  abstract fetchApplication(id: string): Promise<void>
  abstract setApplication(): Promise<void>
  abstract fetchDocumentTemplate(): Promise<void>
  abstract setContent(): void
  abstract totalPages(): number
  abstract otherDataInitiation(): Promise<void>

  async initializeResolution(isInPreviewMode: boolean): Promise<void> {
    ...
    try {
      ...
      await Promise.all([this.initializeApplication(), this.fetchDocumentTemplate(), this.otherDataInitiation()])
  ...
  isSignatureEditable(group: string): boolean {
    ...
    return !this.application.value.signatureGroups.some((sg: SignatureGroup) => {
      return sg.email === this.currentUser.email && sg.group?.target === group
    })
  }

  hasSigned(email: string, group: string): boolean {
    if (!this.application.value || this.application.value.signatureGroups.length <= 0) {
      return false
    }

    return this.application.value.signatureGroups.some((sg: SignatureGroup) => {
      return sg.email === email && sg.group?.target === group
    })
  }

  signatureFile(email: string, group: string): string | null {
    if (!this.application.value || this.application.value.signatureGroups.length <= 0) {
      return null
    }
  ...
  async getPersonsToSign(): Promise<void> {
    if (this.isDcr.value) {
      ...
      this.signatureItems.value = this.shareholderRepository.shareholders.map((s: Shareholder) => {
        ...
        return new SignatureItem(
          this.signatureFile(s.email, "shareholder"),
          this.hasSigned(s.email, "shareholder"),
          this.isSignatureEditable("shareholder"),
  ...
  async updateApplicationContent(updatedApplicationData: T): Promise<void> {
    ...
    let isSame = ObjectUtil.isEqual<T>(this.application.value as T, updatedApplicationData)
    if (isSame) {
      this.setContent()
      return
    }
  ...
  get isSignatureRequired() {
    let currentUserSignatureItem = this.signatureItems.value.find((si: SignatureItem) => {
      return si.email === this.currentUser.email
    })
