...
export class DcrSubscribeBusinessAddressController extends ResolutionController<CompanyAddressSubscription> {
  companyAddressSubscriptionRepository = useCompanyAddressSubscriptionStore()
  ...
  private documentTemplateId: string = "6999e1d3-85f8-4658-af88-b9bd2eaacfe2"

  time = useLocalTime()

  constructor(props: IPropsResolutionDocument<CompanyAddressSubscription>, emitEvents: any | null) {
    super(
      props.companyId,
      props.applicationId,
      props.application,
  ...
  async fetchApplication(id: string): Promise<void> {
    let response = await this.companyAddressSubscriptionRepository.fetch(id)
    if (!this.companyAddressSubscriptionRepository.error && response !== null) {
      this.application.value = new CompanyAddressSubscription(response)
      this.initializeData()
    }
  }
  ...
  async setApplication(): Promise<void> {
    ...
    if (!this.companyRepository.error) {
      this.application.value = new CompanyAddressSubscription()
  ...
  getContent(): string {
    let subscriptionPeriod =
      this.application.value?.subscriptionPeriod === "annually" ? "for another period of one (1) year" : "per quarter"
    let searchValue = "$text.&lt;name=subscriptionPeriod&gt;$"
