...
export class OverviewController {
  ...
  async onGenerateBOAnnexureTxtClicked(): Promise<void> {
    let detailsOfBO = this.shareholders.value.map((s: Shareholder, index: number) => {
      ...
      return `
        ${index + 1}    ${type}    ${s.fullName()}    ${s.location?.addressLine1.toUpperCase()}    ${s.location?.addressLine2?.toUpperCase() ?? ""}        ${s.location?.postcode} ${s.location?.city?.name.toUpperCase() ?? ""}    ${s.location?.country?.name.toUpperCase() ?? ""}     ${s.location?.state?.name.toUpperCase() ?? ""}                                   ${s.email}    ${citizenship}    ${dateOfBirth}    ${race}    ${gender}    ${details.identification}    ${isDirector ? "COMPANY DIRECTOR" : ""}    Direct - Criteria A    ${s.percentageShares}            ${dateAppointed}
        `
    })
