<template>
  <div
    id="company-service-change-of-registered-address"
  ...
  >
    ...
    <Teleport to="body">
      <RegisteredAddressConfirmation
        ...
        @cancel="controller.onPopupCancel()"
        @proceed="controller.onPopupProceed()"
      />
      <HandoverDocumentConfirmation
        ref="handoverPopupRef"
        @cancel="controller.onHandoverCancel()"
        @proceed="controller.onHandoverProceed()"
      />
      <div
        class="alert-tray"
...
<script setup lang="ts">
  ...
  import RegisteredAddressConfirmation from "@/components/Popups/RegisteredAddressConfirmation.vue"
  import HandoverDocumentConfirmation from "@/components/Popups/HandoverDocumentConfirmation.vue"
  import LoaderPrepare from "@/components/Loaders/Prepare.vue"
  import { ChangeOfRegisteredAddressServiceController } from "~/scripts/components/company-services/ChangeOfRegisteredAddressServiceController"
  ...
  const handoverPopupRef = ref(null)

  const controller = new ChangeOfRegisteredAddressServiceController(props.companyId, props.viewType, emit)
  ...
  watch(
    ...
    (newVal) => {
      controller.setConfirmationPopupRef(newVal)
    },
    { immediate: true }
  )
  ...
  watch(
    handoverPopupRef,
    (newVal) => {
      controller.setHandoverPopupRef(newVal)
    },
    { immediate: true }
  )
</script>
