<template>
  <div
    id="company-service-enter-contract"
  ...
  >
    ...
    <Teleport to="body">
      <NoCommonSeal
        ref="noCommonSealRef"
        @proceed="controller.onAdoptCommonSeal()"
      />
      <div
        class="alert-tray"
...
<script lang="ts" setup>
  ...
  import DcrEnterContracts from "../Resolutions/DcrEnterContracts.vue"
  import CompanyServiceWrapper from "@/components/CompanyServices/CompanyServiceWrapper.vue"
  import NoCommonSeal from "../Popups/NoCommonSeal.vue"
  import ActionTray from "../ActionTrays/ActionTray.vue"
  import { EnterContractServiceController } from "~/scripts/components/company-services/EnterContractServiceController"
  ...
  const noCommonSealRef = ref(null)

  const controller = new EnterContractServiceController(props.companyId, props.viewType, emit)
  ...
  watch(
    noCommonSealRef,
    (newVal) => {
      controller.setNoCommonSealRef(newVal)
    },
    { immediate: true }
  )
</script>
...
<style lang="scss">
  ...
  @use "~/assets/scss/components/CompanyServices/EnterContractService" as *;
</style>
