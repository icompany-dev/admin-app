<template>
  ...
  >
    ...
    <Teleport to="body">
      ...
      <CommonSealRequired
        ref="commonSealRequiredRef"
        @proceed="emit('adoptCommonSeal')"
      />
    </Teleport>
  </div>
</template>
...
<script setup lang="ts">
  ...
  import CommonSealRequired from "../Popups/CommonSealRequired.vue"
  import Tooltip from "../Tooltips/Tooltip.vue"
  import { ReplaceCommonSealServiceController } from "~/scripts/components/company-services/ReplaceCommonSealServiceController"
  ...
  const emit = defineEmits([...EmitMessages.COMPANY_SERVICES, "adoptCommonSeal"])

  const dcrRef = ref(null)
  const wrapperRef = ref(null)
  const commonSealRequiredRef = ref(null)

  const controller = new ReplaceCommonSealServiceController(props.companyId, props.viewType, emit)
  ...
  watch(
    commonSealRequiredRef,
    (newVal) => {
      controller.setCommonSealRequiredRef(newVal)
    },
    { immediate: true }
  )
</script>
