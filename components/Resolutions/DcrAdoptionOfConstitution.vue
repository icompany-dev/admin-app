...
<script setup lang="ts">
  ...
  import { DcrAdoptionOfConstitutionController } from "~/scripts/components/resolutions/DcrAdoptionOfConstitutionController"
  import type { IPropsResolutionDocument } from "~/scripts/props/PropsResolutionDocument"

  const props = defineProps<IPropsResolutionDocument>()

  const resolutionContent = ref(null)

  const emit = defineEmits(["startLoading", "doneLoading", "signed"])

  const controller = new DcrAdoptionOfConstitutionController(props, emit)
