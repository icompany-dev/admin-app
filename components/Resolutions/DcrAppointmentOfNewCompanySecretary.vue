<template>
  <div id="dcr-appointment-of-new-company-secretary">
    <!--NOTE: This is a breakaway from the resolution component. It is needed because we need to be able to change the header-->
    <div>
      <div v-for="page in pageRange" class="paper-wrapper" :key="page" :id="`dcr-${page}`">
        <div class="paper resolution">
          <div
            v-if="props.showWatermark"
            class="document-watermark"
            :class="{ preview: props.isOnPreview }"
          >
            <div class="watermark-text">
              {{ props.watermarkText }}
            </div>
          </div>
          <div class="document-ear-mark dcr">
          ...
          <div class="company-detail-head">
            <div class="company-name">
              <InputGroupText :input-value="controller.application.value.name" :is-input-valid="true"
                :is-warning-required="false" :placeholder="'FULL NAME OF YOUR COMPANY'" :pre-or-append-value="'SDN BHD'"
                :is-append-value="true" v-if="controller.isDocumentEditable()" :is-readonly="false"
            ...
            <div class="company-registration-number">
              <InputGroupDualField :first-input-value="controller.application.value.registrationNumberNew"
                :second-input-value="controller.application.value.registrationNumberOld" :is-input-valid="true"
                :is-warning-required="false" :first-placeholder="'123456789'" :second-placeholder="'123456-X'"
                :pre-or-append-value="'Company No:'" :is-append-value="false" :is-prepend-value="true"
                v-if="controller.isDocumentEditable()" :is-readonly="false"
          ...
          <div class="resolution-content">
            <div v-if="page > 1" class="continue-page">
            ...
            <p v-if="!controller.isAppointNew()">
              ...
              <span class="prev-cosec-details" :class="{ 'placeholder': controller.isPlaceholder() }">
                <b>
            ...
            <p>
              ...
              <span :class="{ 'value-placeholder' : controller.showPlaceholder() }">
                <b>
                  ...
                  ({{ controller.getSecretaryLicense() }} / SSM PC NO. {{ controller.getSecretarySsmPcNo() }})
                </b>
              </span>
          ...
          <div class="signature-section" v-if="page >= controller.signatureStartOnPage.value">
            <div class="signature-title">
              {{ controller.signatureTitle() }}
            </div>
            <div class="signature-item" v-for="(signatureItem, index) in signatureController.getSignatureOnPage(page)"
              :key="index">
              <Signature :signature-item="signatureItem"
                :is-readonly="props.isOnPreview"
                @is-enlarged="signatureController.handleEnlargedSignaturePad($event)"
                @signed="emit('signed', $event)" />
            </div>
          </div>
          <div class="resolution-date" v-if="page === controller.totalPages()">
          ...
          <div class="paper-page-number">
            Page {{ page }} of {{ controller.totalPages() }}
          </div>
        </div>
      </div>
    </div>
...
watch(() => controller.signatureItems.value, (newVal) => {
  signatureController.signatureItems.value = newVal
}, { deep: true })

watch(() => controller.maxSignatureOnFirstPage.value, (newVal) => {
