    ...
    >
      <template #page1>
        <p>
          ...
          <br />
          <br />
          THAT the applications for a total of
          <span
            class="fit-content"
            ...
            >
              <td>
                <div
                  ...
                  :class="{ 'align-center': allotTo.type !== AlloteeType.New }"
                >
                  <select
...
<script setup lang="ts">
  ...
  import { McrAllotmentOfSharesController } from "~/scripts/components/resolutions/McrAllotmentOfSharesController"
  ...
  const controller = new McrAllotmentOfSharesController(props, emit)

  watch(
    () => props.companyId,
    (newVal) => {
      controller.setCompanyId(newVal)
      controller.fetchShareholders()
    }
  )
  ...
  defineExpose({
    ...
    updateAllotTos: controller.updateAllotTos.bind(controller),
  })
</script>
