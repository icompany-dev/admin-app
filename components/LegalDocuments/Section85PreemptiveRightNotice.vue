  ...
  <div class="legal-document-section-85">
    ...
    >
      <div class="paper print legal-document portrait section-85">
        ...
        <div class="attention-to">Attention to: {{ controller.shareholderName() }}</div>

        <div class="greeting">Dear Sirs/Madam,</div>

        <div class="title">Notice of Pre-Emptive Rights Pursuant to Section 85 of the Companies Act 2016</div>
        ...
        <div class="content">
          <p>
            ...
            in proportion to your current holdings.
          </p>
          <p>
            If we receive no response from you by
            <b>{{ controller.formattedExpiryDate() }}</b>
            , you hereby automatically consent to waive your pre-emptive rights to the new shares.
          </p>
          <p>Reach out to the Company Secretary at connect@icompany.my.</p>
        </div>
        ...
        <div class="content">
          <p>
            {{ controller.pronoun() }},
            <b>{{ controller.shareholderName() }}</b>
            , acknowledge receipt of the Pre-emptive Rights Notice dated
            {{ controller.formattedNotificationDate() }} (Section 85 of the Companies Act 2016) and:
          </p>
          <div class="form-check">
            <i
              v-if="!controller.canRespond()"
              class="fa-regular"
              :class="controller.isSubscribe() ? 'fa-square-check' : 'fa-square'"
            />
            <input
              v-if="controller.canRespond()"
              ...
              :checked="controller.isSubscribe()"
              @change="controller.onCheckboxClick(true)"
            />
            <label class="mt-1">
              {{ controller.pronoun() }} wish to subscribe to the new shares offered.
              <span
                v-if="controller.hasResponseDate()"
                class="fw-bold"
              >
          ...
          <div class="form-check">
            <i
              v-if="!controller.canRespond()"
              class="fa-regular"
              :class="controller.isWaived() ? 'fa-square-check' : 'fa-square'"
            />
            <input
              v-if="controller.canRespond()"
              type="checkbox"
              class="form-check-input"
              :disabled="!controller.canRespond()"
              :checked="controller.isWaived()"
              @change="controller.onCheckboxClick(false)"
            />
            <label class="mt-1">
        ...
        <div class="signature-container">
          <div class="signature">
            <Signature
              :signature-item="controller.responseSignatureItem.value"
            ...
            >
              Date : {{ controller.formattedResponseDate() }}
            </span>
            <span
              v-if="!controller.hasResponseDate()"
              class="post-signature"
